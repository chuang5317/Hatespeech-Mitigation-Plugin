...
def get_trac(): # 1 2
    ...
    label_col = 2
    for i in range(len(train)):
      train.iloc[i, 2] = 0 if train.iloc[i, 2] == "NAG" else (1 if train.iloc[i, 2] == "CAG" else 2)
    return train
...
bert_model = "./modules/bert"

def create_tokenizer_from_hub_module():
  """Get the vocab file and casing info from the Hub module."""
  with tf.Graph().as_default():
    tokenization_info = hub.KerasLayer(bert_model, trainable=True, signature='tokens', signature_outputs_as_dict=True)
...
# Convert our train and val features to InputFeatures that bert understands.
...
def create_model(is_predicting, input_ids, input_mask, segment_ids, labels,
  ...
  bert_module = hub.KerasLayer(bert_model, trainable=True, signature='tokens', signature_outputs_as_dict=True)
  bert_inputs = dict(
      input_ids=input_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    if is_predicting:
      return (predicted_labelslabels, log_probs)
...
print("len(train_features) is :"+ str(len(train_features)) )
